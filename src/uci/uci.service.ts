...
import { ResponseCode, ResponseStatus, SignupResponse } from './uci.interface';
...
export class UCIService {
  ...
  async login(user: any): Promise<SignupResponse> {
    const fusionAuthUser = (await this.fusionAuthService.login(user)).response;
  ...
  async verifyAndLoginOTP({ phone, status }): Promise<SignupResponse> {
    ...
    if (status === SMSResponseStatus.success) {
      ...
      const {
        ...
        await this.fusionAuthService.getUser(phone);
      if (statusFA === FAStatus.ERROR) {
        // throw new HttpException(
        //   `Error loggin in: ${phone} is not a valid user.`,
        //   HttpStatus.BAD_REQUEST,
        // );
        // console.log('entered inside exception code')
  ...
  async loginTrainee({ id, dob, role }): Promise<SignupResponse> {
    ...
    const {
      ...
      await this.fusionAuthService.getUser(id);
    if (statusFA === FAStatus.ERROR) {
      ...
      if (birthDate === dob && userRoles.indexOf(role) > -1) {
        ...
        throw new HttpException(
          birthDate === dob
            ...
            ? 'Error in logging in: DOB Mismatch'
            : 'Error logging in: Role and DOB Mismatch',
          HttpStatus.BAD_REQUEST,
        );
      }
      const passwordStatus = await this.updatePassword({
