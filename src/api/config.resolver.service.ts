...
export class ConfigResolverService {
  ...
  getLoginExpiryStatus(applicationId: string): boolean {
    applicationId = this.transform(applicationId);
    const config = this.configService.get<string>(applicationId);
    return !!JSON.parse(config).login_expiry?.enabled;
  }

  getLoginExpiryThreshold(applicationId: string): number {
    if (this.getLoginExpiryStatus(applicationId)) {
      applicationId = this.transform(applicationId);
      const config = this.configService.get<string>(applicationId);
      return JSON.parse(config).login_expiry.threshold || undefined;
    }
    throw new Error(
      `login_expiry is not enabled for application: ${applicationId}`,
    );
  }
}
