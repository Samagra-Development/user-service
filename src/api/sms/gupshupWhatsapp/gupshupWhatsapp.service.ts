...
export class GupshupWhatsappService {
  ...
  async sendWhatsappOTP(
    ...
    try {
      ...
      const optinURL =
        process.env.GUPSHUP_WHATSAPP_BASEURL + '?' + optInParams.toString();
      ...
      const otpMessage = process.env.GUPSHUP_WHATSAPP_OTP_TEMPLATE.replace(
        '%otp%',
        `${otp}`,
      );
      ...
      sendOtpParams.append(
        'footer',
        process.env.GUPSHUP_WHATSAPP_OTP_TEMPLATE_FOOTER,
      );

      const sendOtpURL =
        process.env.GUPSHUP_WHATSAPP_BASEURL + '?' + sendOtpParams.toString();
      ...
      if (response.status === 200) {
        ...
        await this.redis.set(
          `whatsapp_otp:+${countryCode.trim()}-${smsData.phone}`,
          otp.toString(),
          'EX',
          1800,
        );
