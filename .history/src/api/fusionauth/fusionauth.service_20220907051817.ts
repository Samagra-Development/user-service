...
export class FusionauthService {
  ...
  login(user: LoginRequest): Promise<ClientResponse<LoginResponse>> {
    ...
    let apiKey = this.configResolverService.getApiKey(user.applicationId)
      if ( apiKey === null && user['authHeader'] != null ) {
          apiKey = user['authHeader']
    }
  ...
  async updateUser(userId: string, user: UserRequest, applicationId: string, authHeader: string): Promise<{_userId: UUID, user: User, err: Error}> {
    let apiKey = this.configResolverService.getApiKey(applicaionId)
      if ( apiKey === null && user['authHeader'] != null ) {
          apiKey = user['authHeader'];
    }
    const host = this.configResolverService.getHost(applicaionId);
    const fusionauthClient = this.getClient(apiKey, host)
    return fusionauthClient
  ...
  async upddatePasswordWithLoginId(data: {loginId: string, password: string, applicationId: string, authHeader?: string}): Promise<any> {
    let apiKey = this.configResolverService.getApiKey(data.applicationId)
      if ( apiKey === null && data.authHeader != null ) {
          apiKey = data.authHeader
    }
    const host = this.configResolverService.getHost(data.applicationId);
